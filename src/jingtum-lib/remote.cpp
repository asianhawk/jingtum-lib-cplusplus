...
jingtumlib::Remote::~Remote() {
	...
	delete _cache;
	delete _paths;
	for (unsigned int i = 0; i < txp.size(); ++i) {
		delete txp[i];
	}	
}
...
void jingtumlib::Remote::_submit(std::string command, std::string data, para pa, std::string(*cbfunc)(std::string, para), void(*cb0)(std::string ex, std::string msg, jingtumlib::Remote *remote), void(*cb1)(std::string, std::string), void(*cb2)(std::string, std::string, int), void(*cb3)(std::string)) {
	server->sendMessage(command, data);
	para2 reqcb;
	reqcb.cbfunc = cbfunc;
	reqcb.callback0 = cb0;
	reqcb.callback1 = cb1;
...
void jingtumlib::Remote::on(std::string option, void(*callback)(std::string)) {
	if (option == "transaction") {
		ontransaction = true;
		_ontransaction = callback;
		subscribe("[\"transactions\"]").submit();
		
	}
	else if (option == "ledger_closed") {		
		onledgerclosed = true;
		_onledgerclosed = callback;
		subscribe("[\"ledger\"]").submit();
	}
	else {
...
void jingtumlib::Remote::_requestAccount(boost::property_tree::ptree pt, Request *req) {
	...
	std::string ledger;
	try {
		ledger = pt.get<std::string>("ledger");
		flag = 1;
	}
	catch (boost::property_tree::ptree_error ex) { flag = 0; }
	if (flag == 1)	req->selectLedger(ledger);
...
jingtumlib::Transaction jingtumlib::Remote::buildPaymentTx(std::string option) {
	...
	tx.id = txp.size();
	jingtumlib::Transaction *tmp = new jingtumlib::Transaction;
	txp.push_back(tmp);
...
void jingtumlib::Remote::_handleLedgerClosed(boost::property_tree::ptree data) {
	if (data.get<int>("ledger_index") > _status.get<int>("ledger_index") && onledgerclosed == true) {
		...
		boost::property_tree::write_json(str, data);
		_onledgerclosed(str.str());
	}
}
...
void jingtumlib::Remote::_handleResponse(boost::property_tree::ptree data) {
	...
	if ( flag != 1 || req_id < 0 || (unsigned int)req_id > server->bmpack.size()) {
		return;
	}
	...
	if ( f1 == 1 && status == "success" && f2 == 1) {
		_updateServerStatus(result);
	}

	std::stringstream res;
	boost::property_tree::write_json(res, result);
	if (status == "success") {
		if (reqcb.cbfunc != NULL) {
			std::string s = reqcb.cbfunc(res.str(), reqcb.pa);			
			if (reqcb.callback0 != NULL) reqcb.callback0("", s, this);
			if (reqcb.callback1 != NULL) reqcb.callback1("", s);
			if (reqcb.callback2 != NULL) {
				reqcb.callback2("", s, (int)txp[reqcb.pa.id]);
			}
			if (reqcb.callback3 != NULL) reqcb.callback3(s);
		}
		else {			
			if (reqcb.callback0 != NULL) reqcb.callback0("", res.str(), this);
			if (reqcb.callback1 != NULL) reqcb.callback1("", res.str());
			if (reqcb.callback2 != NULL) { 
				reqcb.callback2("", res.str(), (int)txp[reqcb.pa.id]);
			}
			if (reqcb.callback3 != NULL) reqcb.callback3(res.str());
		}		
	}
	else if (status == "error") {
		std::string err= "";
		try {
			err = data.get<std::string>("error_message");
		}
		catch(boost::property_tree::ptree_error ex) {}
		try {
			err = data.get<std::string>("error_exception");
		}
		catch (boost::property_tree::ptree_error ex) {}
		if (reqcb.callback0 != NULL) reqcb.callback0(err, "", this);
		if (reqcb.callback1 != NULL) reqcb.callback1(err, "");
		if (reqcb.callback2 != NULL) reqcb.callback2(err, "", (int)txp[reqcb.pa.id]);
		if (reqcb.callback3 != NULL) reqcb.callback3(err);
	}
