...
jingtumlib::Transaction::Transaction() {
}

jingtumlib::Transaction::Transaction(jingtumlib::Transaction *otran) {
	rem = otran->rem;
	tx_json = otran->tx_json;
	_secret = otran->_secret;
	pa = otran->pa;
	_local_sign = otran->_local_sign;
	id = otran->id;
}
...
void jingtumlib::Transaction::submit(void(*callback)(std::string, std::string)) {
	...
	if (found != std::string::npos) {
		...
		if (tx_json.get<std::string>("TransactionType") == "Signer") {
			...
			rem->_submit("submit", data, pa, NULL, NULL, callback, NULL, NULL);
			...
			// store tx
			rem->txp[id]->rem = rem;
			rem->txp[id]->tx_json = tx_json;
			rem->txp[id]->_secret = _secret;
			rem->txp[id]->pa = pa;
			rem->txp[id]->_local_sign = _local_sign;
			rem->txp[id]->id = id;
			...
			rem->_submit("submit", getPureStr(data), pa, NULL, NULL, callback, NULL, NULL);
		}
	}
}
...
void remote_callback_submit(std::string err, std::string blob, int addr) {	
	...
	if (err != "") {
		...
		tx->rem->_submit("submit", data, tx->pa, NULL, NULL,  tx->pa._remcs, NULL, NULL);
	}
}
