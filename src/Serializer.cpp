...
void Serializer::serialize(Serializer *so, boost::property_tree::ptree td, boost::property_tree::ptree obj) {
	...
	//STObjectserialize(so, obj, true); //第三个参数都是true，故将STObjectserialize改成两个参数的函数
}
...
boost::property_tree::ptree Serializer::lookup_type_tx(int id) {
	// nodejs中用id对TransactionTypes取值，似乎会取不到值
	// 所以这里没做。
...
void STObjectserialize(Serializer *so, std::string vals, bool no_marker) {
	...
	//bool no_marker = true;
	std::cout << "no_marker" << no_marker << std::endl;

	std::vector<std::string> keys;

	std::cout << "Come in object" << std::endl;
	...
	for (unsigned int i = 0; i < keys.size(); ++i) {
		...
		if (flag == 1)	std::cout << "keys:" << keys[i] << "\tval:" << str.str() << std::endl;
		else std::cout << "keys:" << keys[i] << "\tval:" << t.data() << std::endl;
		...
		{
			printf("%02d ", (unsigned char)so->buffer[i]);
		}
		std::cout << "pointer:" << so->pointer << std::endl;
...
void serialize(Serializer *so, std::string field_name, std::string valueS) {
	...
	/*boost::property_tree::ptree valueJ;
	if (JSON == true) { 
		std::stringstream str(valueS);
		boost::property_tree::read_json(str, valueJ); 
	}*/
	if (!JSON) {
	...
	std::cout << "STserialized_object_type:\t" << serialized_object_type << std::endl;
	...
	try {
		/*if (JSON == true) {
			std::cout << "11111obj" << std::endl;
			callfun(so, valueJ, index, fe);
		}
		else {
			std::cout << "11111string" << std::endl;
			callfun(so, valueS, index, fe);
		}*/
...
void STHash160serialize(Serializer *so, std::string val, bool no_marker) {
	serializeHex(so, val, true);  // ?? nodejs中为 serializeHex(so, convertHexToByteArray(val), true);
}
...
void STAmountserialize(Serializer *so, std::string val, bool no_marker) {
	...
	if (amount.is_native()) {
		...
		std::cout << "hex" << sbn << std::endl;
		valueBytes = convertHexToByteArray(sbn);
		valueBytes[0] &= 0x3f;
		if (!amount.is_negative()) valueBytes[0] |= 0x40;
		for (unsigned int i = 0; i < valueBytes.size(); i++)
		{			
			printf("%02d ", (unsigned char)valueBytes[i]);
		}
		...
		if (!amount.is_zero()) {
			...
			{
				printf("%02d ", (unsigned char)tmparray[i]);
			}
			std::cout << std::endl;
...
void STArrayserialize(Serializer *so, std::string value, bool no_marker) {
	std::cout << "value in STArray:" << value << std::endl;
	...
	for (unsigned int i = 0; i < keys.size(); ++i) {
		...
		if (Value.size() == 0) {
			...
			if (key == "MemoType" || key == "MemoFormat" ||  key == "MemoData") {
				...
				value = value; // 在transaction中未对memo进行解码，因此此处也就未再次编码
			}
		}
